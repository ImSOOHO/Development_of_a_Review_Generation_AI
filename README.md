...
- LLM-as-a-Judge 방식을 이용하여 1차 모델과 2차모델의 출력값을 정량적으로 비교(Open AI 의 GPT-4-Turbo 모델 API를 이용)
  LLM-as-a-Judge란? LLM API가 한 명의 평가자가 되어 다양한 형태의 텍스트를 정량적으로 평가하는 방식
...
# 6. 회고
- LLM-as-a-Judge 방식의 단일 LLM API 사용: 단일 모델에서 오는 BIAS를 제거하기 위해 다양한 모델을 사용해봐야 했으나 비용문제로 GPT-4-Turbo API만 사용
- 2차 설문조사의 표본 부족: 1차 설문조사에 비해 2차 설문조사의 표본수가 적었음(설문에 대한 보상을 줄인 것이 영향을 미쳤다고 판단)
- 기술 구현의 아쉬움: Langchain, VectorDB 등을 이용한 RAG 구현을 수행했다면 더욱 정확한 정보 제공과 문장 구조 개선이 가능하지 않았을까 하는 아쉬움이 남음
- 리소스 부족의 아쉬움: 로컬 환경에서 모델을 학습시키다 보니, 최신 모델을 사용에 제한이 있었고, GPU 사용 비용 및 저장공간에 대한 부담이 컸음
